...
int telephony_set_characterset_generic_rsp(void *telephony_device, cme_error_t err)
{
	return telephony_generic_rsp(telephony_device, err);
}
	...
	if (NULL != buf) {
		...
		if (buf[7] == '=') {
			if (buf[8] == '?') {
				...
				telephony_set_characterset(device, &buf[8]);
			}
		}
	}
	return 0;
...
int telephony_signal_quality_list_supported_rsp(void *telephony_device,
						cme_error_t err)
{
	struct audio_device *device = telephony_device;
...
static int get_signal_quality(struct audio_device *device, const char *buf)
{
	...
	if (buf[6] == '=')
		telephony_signal_quality_list_supported_rsp(device,
					CME_ERROR_NONE);
	else
