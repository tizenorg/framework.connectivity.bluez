...
static int store_identitykey(bdaddr_t *local, bdaddr_t *peer,
				uint8_t bdaddr_type, unsigned char *key,
				uint8_t master)
{
	GString *newkey;
	...
	g_string_append_printf(newkey, " %d ", master);

	err = write_identitykeys(local, peer, bdaddr_type, newkey->str);
...
int btd_event_irk_notify(bdaddr_t *local, bdaddr_t *peer, uint8_t bdaddr_type,
					uint8_t *key, uint8_t master)
{
	struct btd_adapter *adapter;
	struct btd_device *device;
	...
	ret = store_identitykey(local, peer, bdaddr_type, key, master);
	if (ret != 0) {
		error("Fail to store identity random key");
	}
...
#ifdef __TIZEN_PATCH__
void btd_event_disconn_complete_with_reason(bdaddr_t *local, bdaddr_t *peer,
								uint8_t reason)
{
	struct btd_adapter *adapter;
	struct btd_device *device;
