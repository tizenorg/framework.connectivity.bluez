...
#ifdef __TIZEN_PATCH__
	gboolean advertising;		/* Advertising active */
#endif
...
#ifdef __TIZEN_PATCH__
	/* When page / inquiry scans are both disabled, this is not allowed */
...
{
	struct btd_adapter *adapter = data;
	gboolean old_connectable = adapter->scan_mode != SCAN_DISABLED;
	int err;

	if (connectable == old_connectable)
		return dbus_message_new_method_return(msg);

	err = adapter_ops->set_connectable(adapter->dev_id, connectable);
	if (err < 0)
		return btd_error_failed(msg, strerror(-err));
...
static void session_free(void *data)
{
	...
	if (req->msg) {
		...
		if (dbus_message_is_method_call(req->msg, ADAPTER_INTERFACE,
							"RequestSession")) {
			/* Only in "RequestSession" case method will be called */
			/* to agent, so cancel only in that case*/
...
#ifdef __TIZEN_PATCH__
void adapter_advertising_changed(struct btd_adapter *adapter, gboolean advertising)
{
	const char *path = adapter->path;

	if (adapter->advertising == TRUE && advertising == TRUE) {
		DBG("Already in advertising");
		return;
	}

	adapter->advertising = advertising;

	emit_property_changed(connection, path,
			ADAPTER_INTERFACE, "Advertising",
			DBUS_TYPE_BOOLEAN, &advertising);

	return;
}

void adapter_hardware_error(struct btd_adapter *adapter, uint8_t error_code)
{
	const char *path = adapter->path;

	emit_property_changed(connection, path,
			ADAPTER_INTERFACE, "HardwareError",
			DBUS_TYPE_BYTE , &error_code);

	return;
}

void adapter_tx_timeout_error(struct btd_adapter *adapter)
{
	const char *path = adapter->path;
	uint8_t error_code = 0xFF; /*Dummy Value*/
						...
						DBusMessage *msg, void *data)
{
	...
	err = adapter_ops->set_advertising(adapter->dev_id, enable);
	if (err < 0)
		return btd_error_failed(msg, strerror(-err));
						...
						DBusMessage *msg, void *data)
{
	...
	DBG("advertising interval min %x, max %x, filter %x",
				interval_min, interval_max, filter_policy);

	if (filter_policy > 0x03)
		return btd_error_invalid_args(msg);

	err = adapter_ops->set_advertising_params(adapter->dev_id,
						interval_min, interval_max, (uint8_t) filter_policy);
	if (err < 0)
		return btd_error_failed(msg, strerror(-err));
						...
						DBusMessage *msg, void *data)
{
	...
	err = adapter_ops->set_advertising_data(adapter->dev_id,
							len, value);
	if (err < 0)
		return btd_error_failed(msg, strerror(-err));
						...
						DBusMessage *msg, void *data)
{
	...
	err = adapter_ops->set_scan_rsp_data(adapter->dev_id, len, value);
	if (err < 0)
		return btd_error_failed(msg, strerror(-err));
					...
					DBusMessage *msg, void *data)
{
	...
	err = adapter_ops->add_device_white_list(adapter->dev_id, &addr);
	if (err < 0)
		return btd_error_failed(msg, strerror(-err));
					...
					DBusMessage *msg, void *data)
{
	...
	err = adapter_ops->remove_device_white_list(adapter->dev_id, &addr);
	if (err < 0)
		return btd_error_failed(msg, strerror(-err));
					...
					DBusMessage *msg, void *data)
{
	...
	err = adapter_ops->clear_device_white_list(adapter->dev_id);
	if (err < 0)
		return btd_error_failed(msg, strerror(-err));
...
#ifdef __TIZEN_PATCH__
	/* Advertising */
	dict_append_entry(&dict, "Advertising", DBUS_TYPE_BOOLEAN,
							&adapter->advertising);

	value = adapter->scan_mode != SCAN_DISABLED;
...
#ifdef __TIZEN_PATCH__
int str2buf(const char *str, uint8_t *buf, size_t blen)
{
	int i, dlen;
...
#ifdef __TIZEN_PATCH__
static struct smp_irk_info *get_irk_info(const char *addr, uint8_t bdaddr_type,
							const char *value)
{
	struct smp_irk_info *irk;

	irk = g_new0(struct smp_irk_info, 1);

	str2ba(addr, &irk->bdaddr);
	irk->bdaddr_type = BDADDR_LE_RANDOM;

	str2buf(value, irk->val, sizeof(irk->val));
...
#ifdef __TIZEN_PATCH__
static void create_stored_device_from_irks(char *key, char *value,
							void *user_data)
{
	struct adapter_keys *keys = user_data;
	struct smp_irk_info *info;
	char address[18];
	uint8_t bdaddr_type;
	...
	info = get_irk_info(address, bdaddr_type, value);
	if (info == NULL)
		return;
...
#ifdef __TIZEN_PATCH__
static void smp_irk_free(void *data)
{
	struct smp_irk_info *info = data;
...
#ifdef __TIZEN_PATCH__
	create_name(filename, PATH_MAX, STORAGEDIR, srcaddr, "IdentityResolvingKey");
	textfile_foreach(filename, create_stored_device_from_irks, &keys);

	err = adapter_ops->load_irks(adapter->dev_id, adapter, keys.keys);
	if (err < 0)
		error("Unable to load keys to adapter_ops: %s (%d)",
							strerror(-err), -err);
	g_slist_free_full(keys.keys, smp_irk_free);
	keys.keys = NULL;
#endif
...
#ifdef __TIZEN_PATCH__
int btd_adapter_read_manufacturer(struct btd_adapter *adapter,
				uint16_t *manufacturer)
{
	return adapter_ops->read_manufacturer(adapter->dev_id, manufacturer);
}
#endif
