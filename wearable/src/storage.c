...
#ifdef __TIZEN_PATCH__
int write_identitykeys(bdaddr_t *local, bdaddr_t *peer, uint8_t bdaddr_type,
								const char *key)
{
	char filename[PATH_MAX + 1], addr[20];
	...
	create_filename(filename, PATH_MAX, local, "IdentityResolvingKey");
...
gboolean has_identitykeys(bdaddr_t *local, bdaddr_t *peer, uint8_t bdaddr_type)
{
	char filename[PATH_MAX + 1], key[20], *str;
