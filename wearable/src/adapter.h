...
void adapter_hardware_error(struct btd_adapter *adapter, uint8_t error);
...
#ifdef __TIZEN_PATCH__
	int (*read_manufacturer) (int index, uint16_t *manufacturer);
	int (*set_connectable) (int index, gboolean connectable);
	int (*start_custom_discovery) (int index, uint8_t disc_type);
	int (*set_advertising) (int index, gboolean enable);
	int (*set_advertising_params) (int index, uint16_t interval_min,
							uint16_t interval_max,
							uint8_t filter_policy);
	int (*set_advertising_data) (int index, uint32_t length,
							uint8_t *value);
	int (*set_scan_rsp_data) (int index, uint32_t length, uint8_t *value);
	int (*add_device_white_list) (int index, bdaddr_t *bdaddr);
	int (*remove_device_white_list) (int index, bdaddr_t *bdaddr);
	int (*clear_device_white_list) (int index);
#endif
...
#ifdef __TIZEN_PATCH__
	int (*load_irks) (int index, struct btd_adapter *adapter, GSList *keys);
#endif
};
