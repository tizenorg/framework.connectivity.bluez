...
#ifdef __TIZEN_PATCH__
static void offline_char_write_req_cb(guint8 status, const guint8 *pdu,
...
static void offline_char_write_req(gpointer data, gpointer user_data)
{
	struct characteristic *chr = data;
	GAttrib *attrib = user_data;

	gatt_write_char(attrib, chr->handle, chr->value, chr->vlen,
			offline_char_write_req_cb, chr);
...
#ifdef __TIZEN_PATCH__
	if (gatt->write_req == TRUE) {
		g_slist_foreach(gatt->offline_chars,
				offline_char_write_req, attrib);
		gatt->write_req = FALSE;
		return;
	}
#endif
                       ...
                       gpointer user_data)
{
	...
	if (status != 0) {
		...
		reply = btd_error_failed(gatt->query->msg, "Read Failed");
		goto done;
	}
	...
	if (!dec_read_resp(pdu, plen, value, &vlen)) {
		...
		free(value);
		reply = btd_error_failed(gatt->query->msg, "Protocol error");
		goto done;
	}
				...
				void *data)
{
	...
	if (gatt->attrib) {
		...
		if (gatt_read_char(gatt->attrib, chr->handle, 0,
				read_char_cb, chr) <= 0) {
			DBG("return < 0");
			dbus_message_unref(msg);
			g_free(query);
			return btd_error_failed(msg, "Read Fail");
		}
			...
			gpointer user_data)
{
	...
	if (status != 0) {
		g_printerr("Characteristic write failed: %s\n",
			att_ecode2str(status));
		reply = btd_error_failed(gatt->query->msg, "Write Failed");
		goto done;
	}
...
static DBusMessage *set_value_req(DBusConnection *conn, DBusMessage *msg,
                       DBusMessageIter *iter, struct characteristic *chr)
{
	struct gatt_service *gatt = chr->gatt;
	DBusMessageIter sub;
	...
	if (gatt->attrib) {
		...
		if (gatt_write_char(gatt->attrib, chr->handle, value,
			len, write_char_cb, chr) <= 0) {
			error("Write Fail");
			dbus_message_unref(msg);
			g_free(query);
			return btd_error_failed(msg, "Write Fail");
		}
...
static DBusMessage *set_property_req(DBusConnection *conn,
				DBusMessage *msg, void *data)
{
	struct characteristic *chr = data;
	DBusMessageIter iter;
	...
	if (g_str_equal("Value", property))
		return set_value_req(conn, msg, &sub, chr);
...
static const GDBusMethodTable char_methods[] = {
	...
	{ GDBUS_ASYNC_METHOD("ReadCharacteristic", NULL,
			GDBUS_ARGS({ "value", "ay" }),
			read_char) },
	{ GDBUS_ASYNC_METHOD("SetPropertyRequest",
			GDBUS_ARGS({ "name", "s" }, { "value", "v" }), NULL,
			set_property_req) },
#endif
	{ }
...
#else
		strncpy(chr->type, current_chr->uuid, sizeof(chr->type) - 1);
#endif
