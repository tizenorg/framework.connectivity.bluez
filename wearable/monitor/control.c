...
static void mgmt_new_identity_resolving_key(uint16_t len, const void *buf)
{
	const struct mgmt_ev_new_long_term_key *ev = buf;
	char str[18];

	if (len < sizeof(*ev)) {
		printf("* Malformed New Identity Resolving Key control\n");
		return;
	}

	ba2str(&ev->key.addr.bdaddr, str);

	printf("@ New Identity Resolving Key: %s (%d)\n", str,
							ev->key.addr.type);
...
void control_message(uint16_t opcode, const void *data, uint16_t size)
{
	...
	case MGMT_EV_NEW_REMOTE_IRK:
		mgmt_new_identity_resolving_key(size, data);
		break;
#endif
	default:
