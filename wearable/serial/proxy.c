...
static void disable_proxy(struct serial_proxy *prx)
{
	...
	if (prx->auth_flag) {
		btd_cancel_authorization(&prx->src, &prx->dst);
		prx->auth_flag = FALSE;
	}

	if (prx->pending_call) {
		dbus_pending_call_cancel(prx->pending_call);
		dbus_pending_call_unref(prx->pending_call);
		prx->pending_call = NULL;
	}
#endif
...
static int channel_write(GIOChannel *chan, char *buf, size_t size)
{
	...
	while (wbytes < size) {
		...
		if (written < 0) {
			if (errno == EAGAIN) {
				error("Resourse is busy so sleep for 200 ms");
...
#ifdef __TIZEN_PATCH__
static gboolean rfcomm_watch(GIOChannel *chan, GIOCondition cond, gpointer data)
{
	struct serial_proxy *prx = data;
...
static void newconnection_reply(DBusPendingCall *call, void *data)
{
	...
	rfcomm_watch(prx->rfcomm, G_IO_ERR, prx);

	dbus_error_free(&derr);

done:
	DBG("free pending call");
	dbus_pending_call_unref(call);
	prx->pending_call = NULL;
	dbus_message_unref(reply);
}

static gboolean agent_sendfd(char *agent, int fd,
		DBusPendingCallNotifyFunction notify, void *data)
{
	...
	msg = dbus_message_new_method_call(prx->owner, prx->path,
			"org.rfcomm.agent", "NewConnection");
	ba2str(&prx->dst, address);
	...
	dbus_pending_call_set_notify(call, notify, prx, NULL);
	prx->pending_call = call;
...
static void connect_event_cb(GIOChannel *chan, GError *conn_err, gpointer data)
{
	...
	case TCP_SOCKET_PROXY:
		...
		break;
#ifdef __TIZEN_PATCH__
	case DIRECT_FD_PASSING:
		sk = g_io_channel_unix_get_fd(chan);
		prx->fd = sk;
		agent_sendfd(NULL, sk, newconnection_reply, prx);
		return;
#endif
	default:
		sk = -1;
...
#ifdef __TIZEN_PATCH__
static void free_proxy(struct serial_proxy *proxy)
{
	struct serial_adapter *adapter = proxy->adapter;
...
static void watch_proxy_new(DBusConnection *connection, void *user_data)
{
	struct serial_proxy *proxy = user_data;
...
int start_rfcomm_server(struct serial_adapter *adapter,
				const char *uuid_str, const char *path,
				struct serial_proxy **proxy)
{
	struct serial_proxy *prx;

	prx = g_new0(struct serial_proxy, 1);
	prx->uuid128 = g_strdup(uuid_str);
	prx->type = DIRECT_FD_PASSING;
	adapter_get_address(adapter->btd_adapter, &prx->src);
	prx->adapter = adapter;
	prx->path = g_strdup(path);
	prx->fd = -1;
				...
				DBusMessage *msg, void *data)
{
	...
	err = start_rfcomm_server(adapter, uuid_str, path, &proxy);
	g_free(uuid_str);
	...
	adapter->proxies = g_slist_append(adapter->proxies, proxy);
...
#ifdef __TIZEN_PATCH__
	DBG("Free serial proxies during deactivation");
	g_slist_free_full(adapter->proxies, (GDestroyNotify) free_proxy);
#else
...
static const GDBusMethodTable manager_methods[] = {
	...
	{ GDBUS_METHOD("RegisterProfile",
			...
			NULL,
			register_profile) },
	{ GDBUS_METHOD("UnregisterProfile",
