...
extern struct wbs_opts wbs_opts;
uint32_t hfp_version;
uint32_t hfp_supported_feature;
#endif
...
#ifdef __TIZEN_PATCH__
	sdpfeat = (uint16_t) hfp_supported_feature & 0xFF;
...
#ifdef __TIZEN_PATCH__
	DBG("Always Auto accept HF conn to resolve pending serial auth");
	gateway_auth_cb(NULL, device);
#else
	perr = audio_device_request_authorization(device, server_uuid,
...
static int gateway_server_init(struct audio_adapter *adapter)
{
	...
	sdp_record_t *record;
#ifdef __TIZEN_PATCH__	/* do not request role swith */
							...
							gboolean *enable_sco)
{
...
#ifdef __TIZEN_PATCH__
	wbs_opts.wbs_enable = FALSE;

	wbs_enable = g_key_file_get_boolean(config, "WBspeech",
					"WBSEnable", &err);
	if (err)
		g_clear_error(&err);
	else
		wbs_opts.wbs_enable = wbs_enable;
	...
	if (err)
		...
		wbs_opts.i2s_enable = strtol(str, NULL, 16);
	...
	if (err)
		...
		wbs_opts.is_master = strtol(str, NULL, 16);
	...
	if (err)
		...
		wbs_opts.clock_rate = strtol(str, NULL, 16);
	...
	if (err)
		...
		wbs_opts.pcm_interface_rate = strtol(str, NULL, 16);

	str = g_key_file_get_string(config, "Version",
					"HFPVersion", &err);

	/* Set the default version */
	hfp_version = strtol(str, NULL, 16);

	DBG("Reading SupportedFeature");

	str = g_key_file_get_string(config, "Version",
					"SupportedFeature", &err);
	/* HF Supported feautres  - 16 Bit Value*/
	/*Three-way calling, ECNR, Voice recognition, Wide band speech*/
	/*16 Bit value*/
	/*0th bit - TRUE  -ECNR*/
	/*1st bit - TRUE - Call Waiting*/
	/*2nd bit - TRUE - Clip presentation*/
	/*3rd bit - TRUE - Voice Recognition*/
	/*4th bit - FALSE - Remote Volume*/
	/*5th bit - TRUE - WB*/
	hfp_supported_feature = strtol(str, NULL, 16);
	DBG("Read SupportedFeature");
