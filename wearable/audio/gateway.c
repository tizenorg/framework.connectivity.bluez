...
static struct gateway *gw;
static DBusConnection *gw_conn;
...
#ifdef __TIZEN_PATCH__
	if (old_state > GATEWAY_STATE_CONNECTING &&
	    new_state == GATEWAY_STATE_DISCONNECTED) {
		connected = FALSE;
		emit_property_changed(dev->conn, dev->path,
				AUDIO_GATEWAY_INTERFACE, "Connected",
				DBUS_TYPE_BOOLEAN, &connected);
	} else if (old_state < GATEWAY_STATE_CONNECTED &&
		   new_state == GATEWAY_STATE_CONNECTED) {
		connected = TRUE;
		emit_property_changed(dev->conn, dev->path,
				AUDIO_GATEWAY_INTERFACE, "Connected",
				DBUS_TYPE_BOOLEAN, &connected);
...
#ifdef __TIZEN_PATCH__
	DBG("rfcomm_disconnect_cb cond = %d", cond);
#else
	if (cond & G_IO_NVAL)
		return FALSE;
#endif
...
#ifdef __TIZEN_PATCH__
	connected = (gw->state >= GATEWAY_STATE_CONNECTED);
				...
				DBusMessage *msg, void *data)
{
	...
	g_dbus_remove_watch(gw_conn, gw->agent->watch);
...
#ifdef __TIZEN_PATCH__
static DBusMessage *get_tx_power_level(DBusConnection *conn,
				DBusMessage *msg, void *data)
{
	...
	if (!dbus_message_get_args(msg, NULL, DBUS_TYPE_STRING, &str_addr,
							DBUS_TYPE_INVALID))
		return btd_error_invalid_args(msg);

	str2ba(str_addr, &bdaddr);
	type = 0; /* Read Current Transmit Power Level */
	...
	if (!cr) {
		...
		return btd_error_failed(msg, "Memory alloc failed");
	}
	...
	if (ioctl(dd, HCIGETCONNINFO, (unsigned long) cr) < 0) {
		...
		hci_close_dev(dd);
		free(cr);
		return btd_error_failed(msg, "Unable to get connection info");
	}

	if (hci_read_transmit_power_level(dd, htobs(cr->conn_info->handle), type, &value, 1000) < 0) {
		DBG("HCI read transmit power level request failed");
		hci_close_dev(dd);
		free(cr);
		return btd_error_failed(msg, "Read Tx power request failed");
	}

	DBG("Current transmit power level: [%d]", value);
	...
	if (!reply)
		return NULL;
	dbus_message_append_args(reply, DBUS_TYPE_BYTE, &value,
					DBUS_TYPE_INVALID);
				...
				DBusMessage *msg, void *data)
{
	...
	btd_adapter_read_manufacturer(adapter, &manufacturer);

	/* In now, this command supports only in broadcom chip
	 * https://www.bluetooth.org/en-us/specification/assigned-numbers/
	 * company-identifiers
	*/
	if (manufacturer != 0x000F) {
		error("Not support command, manufacturer %x", manufacturer);
		return btd_error_not_supported(msg);
	}
#endif
	...
	vs = BT_VOICE_TRANSPARENT | BT_VOICE_CVSD_16BIT;
	ret = hci_write_voice_setting(dd, vs, 1000);
	if ( ret < 0 ) {
		return btd_error_failed(msg, strerror(-err));
	}
...
static const GDBusMethodTable gateway_agent_methods[] = {
	...
	{ GDBUS_METHOD("SetNbParameters",
			...
			set_nb_parameters) },
	/* Get tx power level */
	{ GDBUS_METHOD("GetTxPowerLevel",
			GDBUS_ARGS({ "address", "s" }),
			GDBUS_ARGS({ "tx_level", "q" }),
			get_tx_power_level) },
#endif
	{ }
};
...
void gateway_start(struct btd_adapter *adapter)
{
	DBG("+" );
	...
	DBG("adapter_path = [%s]", adapter_path);

	gw_conn = dbus_bus_get(DBUS_BUS_SYSTEM, NULL);
	if (!g_dbus_register_interface(gw_conn,
					adapter_path,
					AUDIO_GATEWAY_INTERFACE,
					gateway_agent_methods, NULL,
					NULL, adapter, NULL)) {
		DBG("g_dbus_register_interface failed ...!" );
	}
