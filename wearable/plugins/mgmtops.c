...
static inline int mgmt_advertising(uint32_t settings)
{
	return (settings & MGMT_SETTING_ADVERTISING) != 0;
}
#endif
...
#ifdef __TIZEN_PATCH__
	old_adv = mgmt_advertising(info->current_settings);
	new_adv = mgmt_advertising(settings);

	if ((!old_power || new_power) && new_adv != old_adv)
		adapter_advertising_changed(adapter, new_adv);
#endif
...
#ifdef __TIZEN_PATCH__
static void load_irks_complete(int sk, uint16_t index, uint8_t status,
							 void *buf, size_t len)
{
	if (index > max_index) {
		error("Unexpected index %u in load_irks_complete complete", index);
		return;
	}

	if (status == MGMT_STATUS_SUCCESS)
		DBG("IRKs loaded for hci");
}
...
#ifdef __TIZEN_PATCH__
	btd_event_disconn_complete_with_reason(&info->bdaddr, &ev->addr.bdaddr,
									reason);
#else
...
#ifdef __TIZEN_PATCH__
static int mgmt_load_irks(int index, struct btd_adapter *adapter, GSList *keys)
{
	char *buf;
	struct mgmt_hdr *hdr;
	struct mgmt_cp_load_remote_irks *cp;
	...
	DBG("hci keys %zu", key_count);

	cp_size = sizeof(*cp) + (key_count * sizeof(*key));

	buf = g_try_malloc0(sizeof(*hdr) + cp_size);
	if (buf == NULL) {
		error("No memory for IRKs for hci");
		return -1;
	}

	hdr = (void *) buf;
	hdr->opcode = htobs(MGMT_OP_LOAD_REMOTE_IRKS);
	...
	DBG("key count: %d", cp->key_count);

	for (l = keys, key = cp->keys; l != NULL; l = g_slist_next(l), key++) {
		struct smp_irk_info *info = l->data;
	...
	DBG("size: %d", sizeof(*hdr) + cp_size);

	if (write(mgmt_sock, buf, sizeof(*hdr) + cp_size) < 0) {
		error("Failed to load IRKs for hci");
		err = -errno;
	} else {
		err = 0;
	}
...
static void mgmt_cmd_complete(int sk, uint16_t index, void *buf, size_t len)
{
	...
	case MGMT_OP_SET_ADVERTISING:
		...
		mgmt_new_settings(sk, index, ev->data, len);
		break;
	case MGMT_OP_SET_ADVERTISING_PARAMS:
		DBG("set_advertising_params");
		break;
	case MGMT_OP_SET_ADVERTISING_DATA:
		DBG("set_advertising_data");
		break;
	case MGMT_OP_SET_SCAN_RSP_DATA:
		DBG("set_scan_rsp_data");
		break;
	case MGMT_OP_LOAD_REMOTE_IRKS:
		DBG("set_scan_rsp_data");
		load_irks_complete(sk, index, ev->status, ev->data, len);
		break;
#endif
...
#ifdef __TIZEN_PATCH__
static void mgmt_new_remote_irk(int sk, uint16_t index, void *buf, size_t len)
{
	struct controller_info *info;
	struct mgmt_ev_new_remote_irk *ev = buf;
	...
	if (len < sizeof(*ev)) {
		error("Too small identity resolving key event");
		return;
	}

	DBG("Controller %u type %u", index, addr->type);

	if (index > max_index) {
		error("Unexpected index %u in identity resolving key event", index);
		return;
	}
	...
	if (ev->store_hint) {
		const struct mgmt_irk_info *key = &ev->key;
		bdaddr_t src;

		adapter_get_address(adapter, &src);

		if (key->master) {
			btd_event_irk_notify(&src, &key->addr.bdaddr,
				key->addr.type, key->val,
				key->master);
		}
	}
}

static void mgmt_hardware_error(int sk, uint16_t index, void *buf, size_t len)
{
	struct btd_adapter *adapter;
	struct controller_info *info;
	struct mgmt_ev_hardware_error *ev = buf;
	char addr[18];

	if (len < sizeof(*ev)) {
		error("Too small mgmt_hardware_error event packet");
		return;
	}

	if (index > max_index) {
		error("Unexpected index %u in mgmt_hardware_error event", index);
		return;
	}
	...
	adapter_hardware_error(adapter, ev->error_code);

}

static void  mgmt_tx_timeout_error(int sk, uint16_t index)
{
	struct btd_adapter *adapter;
	struct controller_info *info;
	char addr[18];
	...
	if (index > max_index) {
		error("Unexpected index %u in mgmt_tx_timeout_error event", index);
		return;
	}
	...
	adapter_tx_timeout_error(adapter);
}
#endif
...
static gboolean mgmt_event(GIOChannel *io, GIOCondition cond, gpointer user_data)
{
	...
	case MGMT_EV_NEW_LONG_TERM_KEY:
		...
		break;
#ifdef __TIZEN_PATCH__
	case MGMT_EV_NEW_REMOTE_IRK:
		mgmt_new_remote_irk(sk, index, buf + MGMT_HDR_SIZE, len);
		break;
	case MGMT_EV_HARDWARE_ERROR:
		mgmt_hardware_error(sk, index, buf + MGMT_HDR_SIZE, len);
		break;
	case MGMT_TX_TIMEOUT_ERROR:
		mgmt_tx_timeout_error(sk, index);
		break;
#endif
	default:
...
static int mgmt_set_advertising(int index, gboolean enable)
{
	DBG("index %d, enable %d", index, enable);

	return mgmt_set_mode(index, MGMT_OP_SET_ADVERTISING, enable);
}

static int mgmt_set_advertising_params(int index, uint16_t interval_min,
						uint16_t interval_max, uint8_t filter_policy)
{
	char buf[MGMT_HDR_SIZE + sizeof(struct mgmt_cp_set_advertising_params)];
	struct mgmt_hdr *hdr = (void *) buf;
	struct mgmt_cp_set_advertising_params *cp = (void *) &buf[sizeof(*hdr)];

	DBG("index %d, interval_min %x, interval_max %x, filter_policy %x",
			index, interval_min, interval_max, filter_policy);

	memset(buf, 0, sizeof(buf));

	hdr->opcode = htobs(MGMT_OP_SET_ADVERTISING_PARAMS);
	hdr->index = htobs(index);
	hdr->len = htobs(sizeof(*cp));
...
static int mgmt_set_advertising_data(int index, uint32_t length,
						uint8_t *value)
{
	char buf[MGMT_HDR_SIZE + sizeof(struct mgmt_cp_set_advertising_data)];
	struct mgmt_hdr *hdr = (void *) buf;
	struct mgmt_cp_set_advertising_data *cp = (void *) &buf[sizeof(*hdr)];

	DBG("index %d, length %d", index, length);

	memset(buf, 0, sizeof(buf));

	hdr->opcode = htobs(MGMT_OP_SET_ADVERTISING_DATA);
	hdr->index = htobs(index);
	hdr->len = htobs(sizeof(*cp));

	memcpy(cp->data, value, length);
...
static int mgmt_set_scan_rsp_data(int index, uint32_t length, uint8_t *value)
{
	char buf[MGMT_HDR_SIZE + sizeof(struct mgmt_cp_set_scan_rsp_data)];
	struct mgmt_hdr *hdr = (void *) buf;
	struct mgmt_cp_set_scan_rsp_data *cp = (void *) &buf[sizeof(*hdr)];

	DBG("index %d, length %d", index, length);

	memset(buf, 0, sizeof(buf));

	hdr->opcode = htobs(MGMT_OP_SET_SCAN_RSP_DATA);
	hdr->index = htobs(index);
	hdr->len = htobs(sizeof(*cp));
...
static int mgmt_add_device_white_list(int index, bdaddr_t *bdaddr)
{
	char buf[MGMT_HDR_SIZE + sizeof(struct mgmt_cp_add_dev_white_list)];
	struct mgmt_hdr *hdr = (void *) buf;
	struct mgmt_cp_add_dev_white_list *cp = (void *) &buf[sizeof(*hdr)];
	char addr[18];
	...
	hdr->opcode = htobs(MGMT_OP_ADD_DEV_WHITE_LIST);
	hdr->index = htobs(index);
	hdr->len = htobs(sizeof(*cp));

	cp = (void *) &buf[sizeof(*hdr)];
	bacpy(&cp->bdaddr, bdaddr);
	cp->bdaddr_type = 0x00; /* Public */
...
static int mgmt_remove_device_white_list(int index, bdaddr_t *bdaddr)
{
	char buf[MGMT_HDR_SIZE + sizeof(struct mgmt_cp_remove_dev_white_list)];
	struct mgmt_hdr *hdr = (void *) buf;
	struct mgmt_cp_remove_dev_white_list *cp = (void *) &buf[sizeof(*hdr)];
	char addr[18];
	...
	hdr->opcode = htobs(MGMT_OP_REMOVE_DEV_WHITE_LIST);
	hdr->index = htobs(index);
	hdr->len = htobs(sizeof(*cp));
...
static int mgmt_clear_device_white_list(int index)
{
	struct mgmt_hdr hdr;

	DBG("hci%d", index);

	hdr.opcode = htobs(MGMT_OP_CLEAR_DEV_WHITE_LIST);
	hdr.len = 0;
	hdr.index = htobs(index);

	if (write(mgmt_sock, &hdr, sizeof(hdr)) < 0) {
		DBG("fail to write mgmt_socket!");
		return -errno;
	}
...
static struct btd_adapter_ops mgmt_ops = {
	...
	.set_advertising = mgmt_set_advertising,
	.set_advertising_params = mgmt_set_advertising_params,
	.set_advertising_data = mgmt_set_advertising_data,
	.set_scan_rsp_data = mgmt_set_scan_rsp_data,
	.add_device_white_list = mgmt_add_device_white_list,
	.remove_device_white_list = mgmt_remove_device_white_list,
	.clear_device_white_list = mgmt_clear_device_white_list,
#endif
