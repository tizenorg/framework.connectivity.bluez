...
#ifdef __TIZEN_PATCH__
static void method_return (struct session_remove_data *data)
{
	DBG("+");
...
{
	DBG("+");
	struct session_remove_data *data = user_data;

	g_obex_set_disconnect_function(obex, NULL, NULL);
	method_return(data);
	release_session(data->session);
	obc_session_unref(data->session);
...
void obc_session_disconnect(struct obc_session *session, DBusMessage *message,
					DBusConnection *connection)
{
	DBG("+");
	GError *err = NULL;
	struct session_remove_data *data = g_new0(struct session_remove_data, 1);

	data->session = obc_session_ref(session);
	data->message = dbus_message_ref(message);
	data->connection = dbus_connection_ref(connection);

	g_obex_disconnect(session->obex, disconnect_cb, data,
