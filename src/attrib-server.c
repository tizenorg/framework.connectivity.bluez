...
static void channel_free(struct gatt_channel *channel)
{
	...
	if (channel->device) {
		device_unset_attrib(channel->device);
					...
					uint16_t handle, const char *name)
{
	...
	if (a->len == 2)
		...
		sdp_uuid32_create(&svc, att_get_u32(a->data));
	}
#endif
	else if (a->len == 16)
...
static gboolean register_core_services(struct gatt_server *server)
{
	...
	server->name_handle = 0x0012;
	bt_uuid16_create(&uuid, GATT_CLIENT_CHARAC_CFG_UUID);

	atval[0] = ATT_CHAR_PROPER_READ;
	att_put_u16(server->name_handle, &atval[1]);
	att_put_u16(GATT_CHARAC_SERVICE_CHANGED, &atval[3]);
	att_put_u16(GATT_CLIENT_CHARAC_CFG_IND_BIT, atval);
...
static uint8_t attrib_get_sc_ccc_info(struct btd_device *device, uint16_t handle)
{
	uint16_t cccval = 0;
	char *filename;
	GKeyFile *key_file;
	...
	filename = btd_device_get_storage_path(device, "ccc_sc");
	if (!filename) {
		warn("Unable to get ccc_sc storage path for device");
		return 0;
	}
