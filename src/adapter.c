...
#ifdef __TIZEN_PATCH__
	if (memcmp(adapter->local_irk, "", MGMT_IRK_SIZE)) {
		key_str[0] = '0';
		key_str[1] = 'x';
						...
						DBusMessage *msg, void *data)
{
	...
	if (mgmt_send(adapter->mgmt, MGMT_OP_SET_ADVERTISING_DATA,
					adapter->dev_id, ADV_DATA_LENGTH_MAX,
					&cp, NULL, NULL, NULL) > 0)
		return dbus_message_new_method_return(msg);
...
static DBusMessage *adapter_set_scan_params(DBusConnection *conn,
						DBusMessage *msg, void *data)
{
	struct btd_adapter *adapter = data;
	...
	if (mgmt_send(adapter->mgmt, MGMT_OP_SET_SCAN_PARAMS,
					adapter->dev_id, sizeof(cp), &cp,
					NULL, NULL, NULL) > 0)
		return dbus_message_new_method_return(msg);
					...
					DBusMessage *msg, void *data)
{
	...
	if (mgmt_send(adapter->mgmt, MGMT_OP_REMOVE_DEV_WHITE_LIST,
					adapter->dev_id, sizeof(cp), &cp,
					NULL, NULL, NULL) > 0)
		return dbus_message_new_method_return(msg);
...
static void load_config(struct btd_adapter *adapter)
{
	...
	if (adapter->le_privacy_enabled) {
		...
		if (gerr || !str || strlen(str) != 34) {
			memcpy(adapter->local_irk, "", MGMT_IRK_SIZE);
			g_error_free(gerr);
			gerr = NULL;
		} else
					...
					const uint8_t *data, uint8_t data_len)
{
	...
	if (bdaddr_type != BDADDR_BREDR &&
		...
		DBG("Remote LE device is non discoverable mode");
...
#ifdef __TIZEN_PATCH__
		/*DBG("List BREDR:%p LE:%p", adapter->discovery_list, adapter->le_discovery_list);*/
	...
	if (bdaddr_type == BDADDR_BREDR)
		...
		device_set_adv_report_info(dev, (void*)data, data_len);
#endif
					...
					const void *param, void *user_data)
{
	...
	DBG("hci%u addr %s, addr_type %d rssi %d flags 0x%04x eir_len %u confirm_name %d legacy %d",
			index, addr, ev->addr.type, ev->rssi, flags, eir_len, confirm_name, legacy);
 ...
 */
		...
		if (device_is_trusted(device) == TRUE ||
			device_is_auto_accept(auth->uuid) == TRUE) {
#else
		if (device_is_trusted(device) == TRUE) {
#endif
					...
					const void *param, void *user_data)
{
	...
	g_dbus_emit_signal(dbus_conn, adapter->path,
			...
			DBUS_TYPE_INT32, &ev->link_type,
			DBUS_TYPE_INT32, &ev->alert_type,
			DBUS_TYPE_INT32, &ev->rssi_dbm,
			DBUS_TYPE_INVALID);
}
					...
					const void *param, void *user_data)
{
	...
	mgmt_register(adapter->mgmt, MGMT_TX_TIMEOUT_ERROR,
						adapter->dev_id,
