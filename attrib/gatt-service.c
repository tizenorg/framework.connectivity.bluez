				...
				uint16_t handle, uint16_t svc, bt_uuid_t *uuid)
{
	...
	if (uuid->type == BT_UUID16) {
		...
		DBG (" 32 bit UUID ");
		att_put_u32(uuid->value.u32, &atval[0]);
		len = 4;
#endif
				...
				uint16_t *handle, struct gatt_info *info)
{
	...
	if (info->uuid.type != BT_UUID16)
		...
		bt_uuid32_create(&bt_uuid, info->uuid.value.u32);
	else
...
#ifdef __TIZEN_PATCH__
	/* characteristic declaration check for UUID Type*/

	if (info->uuid.type == BT_UUID32) {
